...
async def cmd_admin(message: Message, state: FSMContext):
    ...
    await message.answer(MESSAGES['admin_welcome'], reply_markup=await keyboards.admin_menu())

@router.message(AdminState.main, F.text == "🔙 Вийти з адмін-панелі")
async def exit_admin(message: Message, state: FSMContext):
    is_admin = await db.is_user_admin(message.from_user.id)
    await state.clear()
    await message.answer(MESSAGES['admin_exit'], reply_markup=await keyboards.reply_options(is_admin))

@router.message(AdminState.main, F.text == "📊 Статистика")
async def admin_stats(message: Message):
    stats = await db.get_bot_stats()
    await message.answer(MESSAGES['admin_stats'].format(
...
async def admin_broadcast(message: Message, state: FSMContext):
    ...
    await message.answer(MESSAGES['admin_broadcast'], reply_markup=await keyboards.cancel_admin())

@router.message(AdminState.waiting_for_broadcast)
async def admin_broadcast_send(message: Message, state: FSMContext, bot: Bot):
    if message.text == "❌ Скасувати":
        await state.set_state(AdminState.main)
        await message.answer(MESSAGES['admin_broadcast_exit'], reply_markup=await keyboards.admin_menu())
    ...
    await message.answer(MESSAGES['admin_broadcast_info'].format(count=count, blocked=blocked), 
...
async def admin_ban_start(message: Message, state: FSMContext):
    ...
    await message.answer(MESSAGES['admin_ban_prompt'], reply_markup=await keyboards.cancel_admin())

@router.message(AdminState.waiting_for_ban)
async def admin_ban_process(message: Message, state: FSMContext):
    if message.text == "❌ Скасувати":
        await state.set_state(AdminState.main)
        await message.answer(MESSAGES['admin_ban_exit'], reply_markup=await keyboards.admin_menu())
        return
    ...
    try:
        ...
        if await db.is_user_admin(target_id):
            await state.set_state(AdminState.main)
            return await message.answer(MESSAGES['admin_ban_admin_error'].format(user_id=target_id), 
                                      reply_markup=await keyboards.admin_menu())

        await db.deactivate_user(target_id)
        await message.answer(MESSAGES['admin_ban_success'].format(user_id=target_id), 
                             reply_markup=await keyboards.admin_menu())
        await state.set_state(AdminState.main)
    except ValueError:
        await message.answer(MESSAGES['admin_error'].format(user_id=message.text), 
                             reply_markup=await keyboards.admin_menu())
...
async def admin_unban_start(message: Message, state: FSMContext):
    ...
    await message.answer(MESSAGES['admin_unban_prompt'], reply_markup=await keyboards.cancel_admin())

@router.message(AdminState.waiting_for_unban)
async def admin_unban_process(message: Message, state: FSMContext):
    if message.text == "❌ Скасувати":
        await state.set_state(AdminState.main)
        await message.answer(MESSAGES['admin_unban_exit'], reply_markup=await keyboards.admin_menu())
        return

    try:
        target_id = int(message.text)
        await db.activate_user(target_id)
        await message.answer(MESSAGES['admin_unban_success'].format(user_id=target_id), 
                             reply_markup=await keyboards.admin_menu())
        await state.set_state(AdminState.main)
    except ValueError:
        await message.answer(MESSAGES['admin_error'].format(user_id=message.text), 
...
async def admin_quick_ban(callback: CallbackQuery):
    ...
    try:
        ...
        if await db.is_user_admin(target_id):
            await callback.answer(MESSAGES['admin_ban_admin_error'].format(user_id=target_id), show_alert=True)
            return

        await db.deactivate_user(target_id)
        await callback.message.edit_text(MESSAGES['admin_ban_success'].format(user_id=target_id))
...
async def cmd_set_admin(message: Message):
    ...
    if await db.check_user_exists(target_id):
        ...
        await message.answer(MESSAGES['setadmin_success'].format(user_id=target_id), parse_mode="Markdown")
    else:
...
async def show_next_report(message: Message, state: FSMContext):
    ...
    if not reports:
        await message.answer("✅ Активних скарг немає! Все чисто.", reply_markup=await keyboards.admin_menu())
        await state.set_state(AdminState.main)
        return
    ...
    await message.answer(
        text, 
        parse_mode="HTML", 
        reply_markup=await keyboards.admin_report_actions(report['sender_id'], str(report['_id']))
    )
...
async def admin_report_decision(callback: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    if action == "dismiss":
        ...
        if warnings >= 3:
            ...
            try:
                await bot.send_message(target_id, "🚫 Ви отримали 3-тє попередження і були заблоковані.")
            except: pass
            await callback.answer(f"3-й варн. Забанено.")
        else:
            await db.close_report(letter_id, callback.from_user.id, "warned")
            try:
                await bot.send_message(target_id, f"⚠️ Ви отримали попередження ({warnings}/3). Дотримуйтесь правил!")
            except: pass
